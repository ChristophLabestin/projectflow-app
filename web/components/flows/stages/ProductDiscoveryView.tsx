        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full">
            ...
            <div className="col-span-1 flex flex-col h-full bg-white dark:bg-slate-900/50 rounded-2xl border border-surface shadow-sm p-6 relative overflow-hidden">
                ...
                <div className="flex flex-col h-full relative z-10">
                    {/* Header */}
                    <div className="mb-6">
                        <h2 className="text-xl font-extrabold text-main tracking-tight">{t('flowStages.productDiscovery.title')}</h2>
                        <p className="text-xs text-muted mt-1">{t('flowStages.productDiscovery.subtitle')}</p>
                        <div className="h-1 w-10 bg-primary rounded-full mt-3" />
                    </div>

                    {/* Market Info */}
                    <div className="space-y-4 mb-6">
                        <div>
                            <label className="text-xs font-bold text-muted uppercase tracking-wider mb-1.5 block">{t('flowStages.productDiscovery.marketSize.label')}</label>
                            <input
                                type="text"
                                ...
                                className="w-full text-sm bg-surface border border-surface rounded-lg px-3 py-2.5 focus:ring-1 focus:ring-primary outline-none"
                            />
                        </div>
                        <div>
                            <label className="text-xs font-bold text-muted uppercase tracking-wider mb-1.5 block">{t('flowStages.productDiscovery.target.label')}</label>
                            <input
                                type="text"
                    ...
                    <div className="flex-1 flex flex-col min-h-0">
                        <label className="text-xs font-bold text-muted uppercase tracking-wider mb-1.5 flex items-center gap-2">
                        ...
                        <textarea
                            ...
                            className="flex-1 w-full bg-surface border border-surface rounded-lg px-3 py-2.5 focus:ring-1 focus:ring-primary outline-none text-sm resize-none"
            ...
            <div className="col-span-1 flex flex-col h-full bg-white dark:bg-slate-900/50 rounded-2xl border border-surface shadow-sm p-6 overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto space-y-3 pr-1">
                    ...
                    <button
                        onClick={addPersona}
                        className="w-full p-4 rounded-xl border-2 border-dashed border-surface hover:border-primary hover:bg-primary/5 transition-all flex items-center gap-3 group"
                    >
                        <div className="size-10 rounded-full bg-surface-hover group-hover:bg-primary/10 flex items-center justify-center transition-colors">
                            <span className="material-symbols-outlined text-muted group-hover:text-primary text-xl">add</span>
                        </div>
                        <div className="text-left">
                            <p className="text-sm font-medium text-muted group-hover:text-main">{t('flowStages.productDiscovery.personas.add')}</p>
                            <p className="text-xs text-subtle">{t('flowStages.productDiscovery.personas.addHint')}</p>
                        </div>
                    </button>
                            ...
                            >
                                <div className="flex items-start justify-between">
                                    <div className="flex items-center gap-3">
                                        <div className="size-10 rounded-full bg-surface-hover flex items-center justify-center text-lg">
                                            👤
                                        </div>
                                        <div>
                                            <input
                                                ...
                                                className="font-semibold text-sm bg-transparent border-none p-0 focus:ring-0 text-main placeholder:text-subtle w-full"
                                            />
                                            <input
                                                type="text"
                                                value={persona.role}
                                                ...
                                                className="text-xs bg-transparent border-none p-0 focus:ring-0 text-muted placeholder:text-subtle w-full"
                                            />
                                        </div>
                                    </div>
                                    <button
                                ...
                                {selectedPersona === index && (
                                    <div className="mt-4 pt-4 border-t border-surface space-y-3" onClick={(e) => e.stopPropagation()}>
                                        <div>
                                            <label className="text-[10px] font-medium text-muted uppercase mb-1 block">{t('flowStages.productDiscovery.personas.painPoints')}</label>
                                            <textarea
                                                ...
                                                rows={3}
                                                className="w-full text-xs bg-surface border border-surface rounded-lg px-3 py-2 focus:ring-1 focus:ring-primary outline-none resize-none"
                                            />
                                        </div>
                                        <div>
                                            <label className="text-[10px] font-medium text-muted uppercase mb-1 block">{t('flowStages.productDiscovery.personas.goals')}</label>
                                            <textarea
            ...
            <div className="col-span-1 flex flex-col h-full bg-white dark:bg-slate-900/50 rounded-2xl border border-surface shadow-sm p-6 overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto space-y-3 pr-1">
                    ...
                    >
                        ...
                        <div className="text-left">
                            <p className="text-sm font-medium text-muted group-hover:text-main">{t('flowStages.productDiscovery.competitors.add')}</p>
                            <p className="text-xs text-subtle">{t('flowStages.productDiscovery.competitors.addHint')}</p>
                        </div>
                    </button>
                    ...
                    {discoveryData.competitors.length === 0 ? (
                        ...
                        discoveryData.competitors.map((competitor: Competitor, index: number) => (
                            <div key={index} className="p-4 rounded-xl border border-surface hover:border-muted transition-all space-y-3">
                                <div className="flex items-center justify-between">
                                    <input
                                        ...
                                        className="font-semibold text-sm bg-transparent border-none p-0 focus:ring-0 text-main placeholder:text-subtle flex-1"
                                    />
                                    <button
                                ...
                                <div>
                                    <label className="text-[10px] font-medium text-muted uppercase mb-1 block">{t('flowStages.productDiscovery.competitors.strengths')}</label>
                                    <textarea
                                ...
                                <div>
                                    <label className="text-[10px] font-medium text-muted uppercase mb-1 block">{t('flowStages.productDiscovery.competitors.weaknesses')}</label>
                                    <textarea

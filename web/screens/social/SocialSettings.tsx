    ...
    const handleConnect = async (platform: SocialPlatform) => {
        ...
        try {
            ...
            if (error.code === 'auth/multi-factor-auth-required') {
                try {
                    // WORKAROUND: Reconstruct the error object completely to avoid property access issues
                    // or read-only 'customData' on the original FirebaseError.
                    const serverResponse = error.customData?._serverResponse || error.customData?.serverResponse;

                    if (!serverResponse) {
                        throw new Error("Missing serverResponse in MFA error");
                    }

                    const mfaErrorPayload = {
                    ...
                    // Cast to any to bypass strict typing of getMultiFactorResolver which might expect a real FirebaseError instance
                    // but functionally usually only checks properties.
                    const resolver = getMultiFactorResolver(auth, mfaErrorPayload as any);
        ...
        switch (activeTab) {
            case 'accounts':
                return (
                    <div className="space-y-6 animate-fade-in">
                        <div>
                            <h2 className="text-xl font-display font-bold text-main">{t('social.settings.accounts.title')}</h2>
                            <p className="text-sm text-muted">{t('social.settings.accounts.subtitle')}</p>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4">
                            {PLATFORMS.map(platform => {
                                ...
                                return (
                                    <div key={platform.id} className="group relative overflow-hidden bg-card p-6 rounded-2xl border border-surface hover:border-primary/50 transition-all">
                                        ...
                                        <div className="relative z-10 flex items-start justify-between">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    <h3 className="font-bold text-lg text-main">{platform.name}</h3>
                                                    {isConnected ? (
                                                        ...
                                                        <p className="text-xs text-muted font-medium mt-1">{t('social.settings.accounts.notConnected')}</p>
                                                    )}
                                                </div>
                                            </div>
                ...
                return (
                    <div className="space-y-6 animate-fade-in">
                        <div className="flex items-center justify-between mb-2">
                            <div>
                                <h2 className="text-xl font-display font-bold text-main">{t('social.settings.presets.title')}</h2>
                                <p className="text-sm text-muted">{t('social.settings.presets.subtitle')}</p>
                            </div>
                            <Button
                                variant="primary"
                                size="sm"
                        ...
                        <div className="pt-4">
                            {presets.length === 0 ? (
                                <div className="p-20 text-center bg-card rounded-2xl border border-dashed border-surface">
                                    <span className="material-symbols-outlined text-4xl mb-4 text-muted">bookmark_add</span>
                                    <h3 className="font-bold text-lg mb-1">{t('social.settings.presets.empty.title')}</h3>
                                    <p className="text-muted mb-6">{t('social.settings.presets.empty.subtitle')}</p>
                                    <Button variant="secondary" onClick={() => setShowPresetManager(true)}>{t('social.settings.presets.empty.action')}</Button>
                                </div>
                            ) : (
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
                                    {presets.map(preset => (
                                        <div key={preset.id} className="group bg-card border border-surface rounded-2xl p-5 hover:border-primary/50 transition-all flex flex-col h-full shadow-sm hover:shadow-xl">
                                            ...
                                            <h4 className="font-black text-lg text-main mb-2 line-clamp-1">{preset.name}</h4>
                                            <p className="text-sm text-muted line-clamp-3 mb-4 flex-1">{preset.content}</p>
                                            <div className="flex items-center gap-2 mt-auto pt-4 border-t border-surface">
            ...
            case 'strategy':
                return (
                    <div className="space-y-6 animate-fade-in">
                        <div>
                            <h2 className="text-xl font-display font-bold text-main">{t('social.settings.strategy.title')}</h2>
                            <p className="text-sm text-muted">{t('social.settings.strategy.subtitle')}</p>
                        </div>
                            ...
                            <Card className="p-8 space-y-8">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                                    <div className="space-y-4">
                                        <label className="text-xs font-black text-muted uppercase tracking-widest">{t('social.settings.strategy.defaultPlatforms')}</label>
                                                ...
                                                return (
                                                    <button
                                                        ...
                                                        className={`size-12 rounded-xl border-2 transition-all flex items-center justify-center overflow-hidden p-0.5 ${isActive
                                                            ? 'border-primary grayscale-0 shadow-md scale-110'
                                                            : 'border-surface grayscale hover:grayscale-[0.5]'
                                                            }`}
                                                    >
                                    ...
                                    <div className="space-y-4">
                                        <label className="text-xs font-black text-muted uppercase tracking-widest">{t('social.settings.strategy.preferredTone')}</label>
                                            ...
                                            {[
                                                ...
                                                <button
                                                    ...
                                                    className={`px-4 py-2 text-xs font-bold border rounded-lg transition-all ${strategy?.preferredTone === tone.id
                                                        ...
                                                        : 'border-surface text-muted hover:border-primary/50'
                                                        }`}
                                                >
                                                    {tone.label}
                                                </button>
                                ...
                                <div className="pt-8 border-t border-surface">
                                    <div className="mb-4">
                                        <h3 className="font-bold text-main">{t('social.settings.strategy.hashtagLimits.title')}</h3>
                                        <p className="text-sm text-muted">{t('social.settings.strategy.hashtagLimits.subtitle')}</p>
                                    </div>
                                            ...
                                            return (
                                                <div key={platform.id} className="space-y-2">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <span className="text-[10px] font-black uppercase tracking-wider text-muted">{platform.id}</span>
                                                    </div>
                                                    <input
                                                        type="number"
                                                        value={limit}
                                                        ...
                                                        className="w-full px-3 py-2 bg-surface rounded-lg border border-surface text-main text-sm focus:ring-1 focus:ring-primary outline-none"
                                                        min="0"
                                                        max="100"
                                                    />
                                                </div>
                                ...
                                <div className="pt-8 border-t border-surface">
                                    <div className="flex items-center justify-between mb-4">
                                        <div>
                                            <h3 className="font-bold text-main">{t('social.settings.strategy.brandPillars.title')}</h3>
                                            <p className="text-sm text-muted">{t('social.settings.strategy.brandPillars.subtitle')}</p>
                                        </div>
                                    </div>
                                    <textarea
                                        ...
                                        className="w-full p-6 bg-surface rounded-xl border border-surface text-main text-sm resize-none focus:ring-1 focus:ring-primary outline-none min-h-[120px]"
                                    />
                                </div>
                            </Card>
                        </div>
                ...
                return (
                    <div className="space-y-6 animate-fade-in">
                        <div>
                            ...
                            <p className="text-sm text-muted">{t('social.settings.danger.subtitle')}</p>
                        </div>
                        ...
                        <div className="pt-4">
                            <Card className="divide-y divide-[var(--color-surface-border)]">
                                <div className="p-6 flex items-center justify-between gap-6">
                                    <div>
                                        <h3 className="font-bold text-main">{t('social.settings.danger.reset.title')}</h3>
                                        <p className="text-sm text-muted mt-1">{t('social.settings.danger.reset.description')}</p>
                                    </div>
                                ...
                                <div className="p-6 flex items-center justify-between gap-6">
                                    <div>
                                        <h3 className="font-bold text-main">{t('social.settings.danger.cleanup.title')}</h3>
                                        <p className="text-sm text-muted mt-1">{t('social.settings.danger.cleanup.description')}</p>
                                    </div>
        ...
        <div className="max-w-6xl mx-auto p-4 md:p-8 animate-fade-up min-h-screen">
            <div className="flex flex-col md:flex-row gap-8 items-start">
                <aside className="w-full md:w-64 shrink-0 space-y-1">
                    <h1 className="text-2xl font-display font-bold text-main px-2 mb-6">{t('social.settings.title')}</h1>

...
import { createSocialPost, deleteSocialPost, getSocialPostById, subscribeCampaigns, updateSocialPost } from '../../services/domain/socialService';
...
export const CreateSocialPost = () => {
    ...
    useEffect(() => {
        ...
        if (paramVisual) {
            setScriptOutline(prev => prev ? prev : `${t('social.postForm.video.visualDirectionLabel')}: ${paramVisual}`);
    ...
    const STEPS = [
        { id: 1, label: t('social.postForm.steps.strategy') },
        { id: 2, label: t('social.postForm.steps.content') },
        { id: 3, label: t('social.postForm.steps.review') }
    ];
        ...
        try {
            ...
            const data: any = {
                ...
                videoConcept: shouldSaveVideoConcept ? {
                    title: videoTitle || (isYouTube ? caption : (caption?.slice(0, 50) || t('social.postForm.video.untitled'))),
                    thumbnailIdea,
                    scriptOutline,
                    thumbnailUrl
                } : null
                    ...
                    <div className="bg-card">
                        {rejectionReason && (
                            <div className="bg-red-50 dark:bg-red-900/20 border-b border-red-100 dark:border-red-900/30 px-8 py-3 flex items-start gap-3">
                                ...
                                <div>
                                    <h4 className="text-sm font-bold text-red-700 dark:text-red-400">{t('social.postForm.changesRequested')}</h4>
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-subtle">
                                        {t('social.postForm.stepLabel')
                                            ...
                                            .replace('{label}', STEPS[currentStep - 1].label)}
                                    </p>
                                </div>
                                {postId && (
                                    <button
                            ...
                            <div className="space-y-6 animate-fade-in">
                                {isYouTube ? (
                                    <div className="space-y-6">
                                        <Input
                                            label={t('social.postForm.video.titleLabel')}
                                            value={videoTitle}
                                            onChange={e => setVideoTitle(e.target.value)}
                                            placeholder={t('social.postForm.video.titlePlaceholder')}
                                            autoFocus
                                        />
                                            ...
                                            <div className="flex gap-4">
                                                ...
                                                <div className="flex-1">
                                                    <Textarea
                                                        ...
                                                        placeholder={t('social.postForm.video.thumbnailPlaceholder')}
                                        ...
                                        <div>
                                            <div className="flex justify-between items-center mb-2">
                                                ...
                                                >
                                                    ...
                                                    {t('social.postForm.video.generateScript')}
                                                </button>
                                            </div>
                                            <Textarea
                                                value={scriptOutline}
                                                onChange={e => setScriptOutline(e.target.value)}
                                                placeholder={t('social.postForm.video.scriptPlaceholder')}
                                            ...
                                            <div className="relative">
                                                <Textarea
                                                    ...
                                                    placeholder={platform === 'X'
                                                        ? t('social.postForm.caption.placeholderX')
                                                        : t('social.postForm.caption.placeholder')}
                                            ...
                                            <div className="flex justify-between items-center">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    {isOverLimit && (
                                                        <span className="flex items-center gap-1 text-[10px] font-black text-red-600 dark:text-red-400 animate-pulse bg-red-50 dark:bg-red-900/40 px-1.5 py-0.5 rounded">
                                                            ...
                                                            {t('social.postForm.hashtags.limitExceeded')
                                                                .replace('{current}', String(currentHashtagCount))
                                                                .replace('{limit}', String(getHashtagLimit(platform)))}
                                                        </span>
                                                    )}
                                                </div>
                                                <div className="flex items-center gap-3">
                                                    {isOverLimit && (
                                                        ...
                                                        >
                                                            <span className="material-symbols-outlined text-[14px]">auto_awesome</span>
                                                            {t('social.postForm.hashtags.aiRework')}
                                                        </button>
                                                    )}
                                                    <button
                                                        onClick={async () => {
                                                            ...
                                                            try {
                                                                const limit = getHashtagLimit(platform);
                                                                const tags = await generateSocialHashtags(caption || videoTitle || t('social.postForm.hashtags.fallbackTopic'), platform, limit);
                                                    ...
                                                    >
                                                        {isOverLimit ? t('social.postForm.hashtags.regenerate') : t('social.postForm.hashtags.generate')}
                                                    </button>
                                                </div>
                                            </div>
                    ...
                    <footer className="px-8 py-5 border-t border-surface flex items-center justify-between bg-card">
                        {currentStep > 1 ? (
                            <Button variant="ghost" onClick={() => setCurrentStep(prev => prev - 1)}>
                                ...
                                {t('social.postForm.actions.back')}
                            </Button>
                        ) : (
                            <Button variant="ghost" onClick={() => navigate(-1)}>{t('social.postForm.actions.cancel')}</Button>
                        )}
                        ...
                        <div className="flex gap-3">
                            ...
                            {currentStep < 3 ? (
                                <Button variant="primary" onClick={() => setCurrentStep(prev => prev + 1)}>
                                    {t('social.postForm.actions.next')}
                                ...
                                <Button
                                    ...
                                    title={(!scheduledDate || !scheduledTime) ? t('social.postForm.actions.scheduleRequired') : ""}
                                >
                                    {t('social.postForm.actions.sendReview')}
                                </Button>
                            )}
                        </div>
                    </footer>
                ...
                <div className="w-[50%] bg-surface flex flex-col items-center justify-center p-8 relative">
                    ...
                    <div className="mb-6 text-center z-10">
                        ...
                        <div className="flex items-center justify-center gap-2 text-xs text-muted">
                            ...
                            {platform} • {getSocialPostFormatLabel(format, t)}
                        </div>
                    </div>
                    ...
                    <div className="mt-8 flex items-center gap-6 text-muted text-[11px] font-medium animate-fade-in">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <span className="uppercase tracking-widest text-[9px] font-bold w-14 inline-block">
                                {isStoryPlaying
                                    ? (isPreviewHovered ? t('social.postForm.preview.paused') : t('social.postForm.preview.playing'))
                                    : t('social.postForm.preview.stopped')}
                            </span>
                        </div>
                    </div>
                </div>

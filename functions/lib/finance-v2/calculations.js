...
exports.calculateScenarioSnapshotFromInput = void 0;
const shared_1 = require("./shared");
const resolveCostAmount = (item) => {
    const quantityPerUnit = (0, shared_1.toNonNegative)(item.quantityPerUnit);
    const unitCost = (0, shared_1.toNonNegative)(item.unitCost);
    if (quantityPerUnit > 0 && unitCost > 0) {
        return quantityPerUnit * unitCost;
    }
    return (0, shared_1.toNonNegative)(item.amount);
};
const resolveCostTokens = (item) => {
    const quantityPerUnit = (0, shared_1.toNonNegative)(item.quantityPerUnit);
    const tokensPerUsage = (0, shared_1.toNonNegative)(item.tokensPerUsage);
    if (quantityPerUnit > 0 && tokensPerUsage > 0) {
        return quantityPerUnit * tokensPerUsage;
    }
    return 0;
};
const sumAmounts = (items) => items.reduce((sum, item) => sum + resolveCostAmount(item), 0);
const sumTokens = (items) => items.reduce((sum, item) => sum + resolveCostTokens(item), 0);
const calculateScenarioSnapshotFromInput = (scenario) => {
    const plannedUnitsRaw = (0, shared_1.toNonNegative)(scenario.plannedUnits);
    ...
    const pricePerUnit = (0, shared_1.toNonNegative)(scenario.pricePerUnit);
    const tokenQuotaPerUnit = (0, shared_1.toNonNegative)(scenario.tokenQuotaPerUnit);
    const discountPercent = (0, shared_1.toNonNegative)(scenario.discountPercent);
    const salesCommissionPercent = (0, shared_1.toNonNegative)(scenario.salesCommissionPercent);
    const targetProfitPercentOnCost = (0, shared_1.toNonNegative)(scenario.targetProfitPercentOnCost);
...
exports.calculateScenarioSnapshotFromInput = calculateScenarioSnapshotFromInput;

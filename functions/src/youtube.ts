...
const CLIENT_SECRET = 'YOUR_YOUTUBE_CLIENT_SECRET';
...
const getOAuthClient = () => {
    return new OAuth2Client(
        CLIENT_ID,
        CLIENT_SECRET,
        getFunctionUrl('youtubeCallback')
    );
};

export const getYouTubeAuthUrl = functions.region('europe-west3').https.onCall(async (data, context) => {
    if (!context.auth) {
...
export const youtubeCallback = functions.region('europe-west3').https.onRequest(async (req, res) => {
    ...
    try {
        ...
        if (!tokens.refresh_token && !existingQuery.empty) {
            delete (integrationData as any).refreshToken;
        }

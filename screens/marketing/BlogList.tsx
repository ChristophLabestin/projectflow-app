...
import { fetchExternalBlogPosts, BlogPost } from '../../services/blogService';
    ...
    React.useEffect(() => {
        const loadBlogs = async () => {
            ...
            try {
                ...
                console.warn('Failed to fetch external blogs, falling back to mock data', err);
                setError('Could not fetch posts from external source. Showing local drafts.');
            } finally {
                setIsLoading(false);
            }
        };
                        ...
                        >
                            ...
                            <div className="flex-1 p-4 flex flex-col">
                                ...
                                <div className="flex items-center justify-between text-xs text-[var(--color-text-muted)] mt-4">
                                    ...
                                    <span>{format(blog.createdAt, 'MMM d, yyyy')}</span>
                                </div>
                            </div>
                        </div>
                    ))}

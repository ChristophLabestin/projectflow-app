...
export const PropertiesPanel = ({ block, parentType, columnCount, onChange, onSaveAsComponent, projectId, variables }: { block: EmailBlock, parentType?: string, columnCount?: number, onChange: (updates: Partial<EmailBlock>) => void, onSaveAsComponent: (name: string) => void, projectId: string, variables?: TemplateVariable[] }) => {
    ...
    const typeIcons: Record<string, string> = {
        text: 'notes', header: 'title', button: 'smart_button', image: 'image',
    ...
    return (
        <div className="flex flex-col h-full bg-[var(--color-surface-paper)] font-sans border-l border-[var(--color-surface-border)] shadow-2xl w-[320px]">
            ...
            <header className="shrink-0 sticky top-0 z-20 px-4 py-5 border-b border-[var(--color-surface-border)] bg-[var(--color-surface-paper)]/80 backdrop-blur-md">
                <div className="flex items-center gap-3.5">
                    <div className="relative shrink-0">
                        ...
                        <div className="absolute -bottom-1 -right-1 size-4 rounded-full bg-[var(--color-primary)] text-white flex items-center justify-center border-2 border-[var(--color-surface-paper)] shadow-sm">
            ...
            {/* Tabs Orchestration */}
            <div className="shrink-0 p-3 bg-[var(--color-surface-sunken)]/30 border-b border-[var(--color-surface-border)]">
                <div className="relative flex p-1 bg-[var(--color-surface-sunken)] rounded-lg border border-[var(--color-surface-border)] shadow-inner">
                    {/* Sliding Highlight */}
                    <div
                        className="absolute h-[calc(100%-8px)] rounded-md bg-white dark:bg-zinc-700 shadow-sm transition-all duration-300 ease-[cubic-bezier(0.34,1.56,0.64,1)]"
                        style={{
                            width: 'calc(50% - 4px)',
                            left: activeTab === 'settings' ? '4px' : 'calc(50%)',
                        }}
                    />

                    <button
                        onClick={() => setActiveTab('settings')}
                        className={`relative z-10 flex-1 flex items-center justify-center gap-2 py-2 text-[10px] font-black uppercase tracking-widest transition-colors ${activeTab === 'settings' ? 'text-[var(--color-primary)]' : 'text-[var(--color-text-muted)]'}`}
                    >
                    ...
                    <button
                        ...
                        className={`relative z-10 flex-1 flex items-center justify-center gap-2 py-2 text-[10px] font-black uppercase tracking-widest transition-colors ${activeTab === 'style' ? 'text-[var(--color-primary)]' : 'text-[var(--color-text-muted)]'}`}
                    >
            ...
            <main className="flex-1 overflow-y-auto custom-scrollbar" role="tabpanel">
                        ...
                        {block.type === 'text' && (
                            <SettingsGroup title="Content" defaultOpen={true}>
                                <div className="space-y-3">
                                    <RichTextEditor value={block.content.text || ''} onChange={html => updateContent('text', html)} variables={variables} />
                                </div>
                            </SettingsGroup>
                        )}
                        ...
                        {block.type === 'image' && (
                            <SettingsGroup title="Image Asset" defaultOpen={true}>
                                <div className="space-y-4">
                                    ...
                                    <SettingsInput label="Source" icon="image" value={block.content.src} onChange={(v: string) => updateContent('src', v)} placeholder="https://..." />
                ...
                {activeTab === 'style' && (
                    <div className="pb-32">
                        ...
                        <SettingsGroup title="Size & Position" defaultOpen={true}>
                        ...
                        <SettingsGroup title="Typography" defaultOpen={['text', 'header', 'button', 'menu'].includes(block.type)}>
            ...
            <footer className="shrink-0 p-4 border-t border-[var(--color-surface-border)] bg-[var(--color-surface-sunken)]/50">
                {!isSaving ? (
                    <button onClick={() => setIsSaving(true)} className="w-full flex items-center justify-center gap-2 py-2 rounded border border-[var(--color-primary)]/30 text-[var(--color-primary)] text-[10px] font-black uppercase tracking-widest hover:bg-[var(--color-primary)] hover:text-white transition-all shadow-sm">
                    ...
                    <div className="space-y-2 animate-in slide-in-from-bottom-2 duration-300">
                        ...
                        <div className="flex gap-2">
                            ...
                            <button onClick={() => { if (compName) { onSaveAsComponent(compName); setIsSaving(false); setCompName(''); } }} disabled={!compName} className="flex-1 py-1.5 text-[9px] font-black uppercase tracking-widest bg-[var(--color-primary)] text-white rounded shadow-lg shadow-[var(--color-primary)]/20 disabled:opacity-30">Confirm</button>
                        </div>
                    </div>
                )}
            </footer>
            ...
            {/* Modals */}
            <MediaLibrary isOpen={isMediaLibraryOpen} onClose={() => setIsMediaLibraryOpen(false)} onSelect={(asset) => { updateContent('src', asset.url); if (!block.content.alt) updateContent('alt', asset.name); }} projectId={projectId} />

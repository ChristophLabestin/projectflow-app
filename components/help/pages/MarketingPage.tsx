...
const MetricCard = ({ label, description, icon, color }: { label: string; description: string; icon: string; color: string }) => (
    <div className="rounded-2xl border border-surface bg-card p-5 text-center">
        ...
        <div className="text-sm font-bold text-main mt-3">{label}</div>
...
const StepBlock = ({ step, title, children }: { step: string; title: string; children: React.ReactNode }) => (
    <div className="rounded-2xl border border-surface bg-card p-5">
        <div className="flex items-center justify-between mb-3"><span className="text-sm font-bold text-main">{title}</span><span className="px-2 py-0.5 rounded-full bg-surface-hover text-[10px] font-bold text-muted">{step}</span></div>
...
export const MarketingPage = (_props: HelpCenterPageProps) => (
    <div className="px-6 py-6 space-y-10">
        <div className="rounded-[28px] border border-surface bg-gradient-to-br from-[var(--color-surface-card)] to-[var(--color-surface-bg)] p-6">
            <div className="text-[10px] font-bold uppercase tracking-[0.28em] text-muted">Marketing</div>
            <h2 className="text-3xl font-bold text-main mt-3">Build Campaigns with a Clear Funnel</h2>
            <p className="text-sm text-muted mt-3 leading-relaxed max-w-2xl">Marketing tools help you plan, produce, and measure outreach. Use the builder for email, manage recipients centrally, and track paid ad performance.</p>
        </div>

        <section data-section-id="email-builder" className="help-section rounded-3xl border border-surface bg-surface p-6 space-y-6">
            <div><div className="text-[10px] font-bold uppercase tracking-[0.28em] text-muted">Create</div><h3 className="text-xl font-bold text-main mt-2">Email Builder</h3><p className="text-sm text-muted mt-2">Build emails with modular blocks for consistent layouts.</p></div>
        ...
        <section data-section-id="templates" className="help-section rounded-3xl border border-surface bg-card p-6 space-y-6">
            <div><div className="text-[10px] font-bold uppercase tracking-[0.28em] text-muted">Start Fast</div><h3 className="text-xl font-bold text-main mt-2">Templates</h3><p className="text-sm text-muted mt-2">Pre-built templates for common use cases.</p></div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {[['Newsletter', 'Regular updates with multiple sections', 'newspaper'], ['Announcement', 'Single-purpose launch or news', 'campaign'], ['Promotional', 'Sales, offers, and discounts', 'sell'], ['Welcome', 'Onboarding new subscribers', 'waving_hand'], ['Event', 'Invitations and reminders', 'event'], ['Digest', 'Weekly or monthly roundups', 'summarize']].map(([n, d, i]) => <div key={n} className="rounded-2xl border border-surface bg-surface p-4"><div className="flex items-center gap-3"><span className="material-symbols-outlined text-[20px] text-primary">{i}</span><span className="text-sm font-bold text-main">{n}</span></div><p className="text-xs text-muted mt-2">{d}</p></div>)}
            </div>
        </section>

        <section data-section-id="recipients" className="help-section rounded-3xl border border-surface bg-surface p-6 space-y-6">
            <div><div className="text-[10px] font-bold uppercase tracking-[0.28em] text-muted">Audience</div><h3 className="text-xl font-bold text-main mt-2">Recipients and Segments</h3><p className="text-sm text-muted mt-2">Organize who receives each campaign.</p></div>
        ...
        <section data-section-id="variables" className="help-section rounded-3xl border border-surface bg-card p-6 space-y-6">
            <div><div className="text-[10px] font-bold uppercase tracking-[0.28em] text-muted">Personalize</div><h3 className="text-xl font-bold text-main mt-2">Variables and Personalization</h3></div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="rounded-2xl border border-surface bg-surface p-5">
                    <div className="text-xs font-bold uppercase tracking-wider text-muted mb-3">Variable Syntax</div>
                    <div className="space-y-2 text-sm">{[['{{first_name}}', 'Recipient first name'], ['{{company}}', 'Company name'], ['{{unsubscribe_url}}', 'Unsubscribe link'], ['{{current_year}}', 'Dynamic year']].map(([v, d]) => <div key={v} className="flex justify-between py-2 border-b border-surface last:border-0"><code className="text-primary text-xs">{v}</code><span className="text-muted">{d}</span></div>)}</div>
                </div>
                <div className="rounded-2xl border border-surface bg-surface p-5">
                    <div className="text-xs font-bold uppercase tracking-wider text-muted mb-3">Best Practices</div>
                    <div className="space-y-3 text-sm text-muted">
                        <div className="flex items-start gap-2"><span className="material-symbols-outlined text-[16px] text-primary">check</span>Always set fallback values for missing data</div>
                        <div className="flex items-start gap-2"><span className="material-symbols-outlined text-[16px] text-primary">check</span>Test with sample recipients before sending</div>
                        <div className="flex items-start gap-2"><span className="material-symbols-outlined text-[16px] text-primary">check</span>Preview how variables render on mobile</div>
                    </div>
                </div>
            </div>
        </section>

        <section data-section-id="paid-ads" className="help-section rounded-3xl border border-surface bg-surface p-6 space-y-6">
            <div><div className="text-[10px] font-bold uppercase tracking-[0.28em] text-muted">Advertising</div><h3 className="text-xl font-bold text-main mt-2">Paid Ads Tracking</h3><p className="text-sm text-muted mt-2">Track ad initiatives and outcomes.</p></div>
            <div className="rounded-2xl border border-surface bg-card p-5">
                <div className="text-xs font-bold uppercase tracking-wider text-muted mb-3">Ad Fields</div>
                <div className="space-y-2">{[['Platform', 'Google, Meta, LinkedIn, TikTok, etc.'], ['Budget', 'Allocated spend for this initiative'], ['Spend', 'Actual spend to date'], ['Goal', 'Target outcome (clicks, signups, sales)'], ['Status', 'Active, Paused, Completed'], ['Notes', 'Learnings and observations']].map(([f, d]) => <div key={f} className="flex justify-between py-2 border-b border-surface last:border-0"><span className="text-sm font-medium text-main">{f}</span><span className="text-sm text-muted">{d}</span></div>)}</div>
            </div>
        </section>

        <section data-section-id="performance" className="help-section rounded-3xl border border-surface bg-card p-6 space-y-6">
            <div><div className="text-[10px] font-bold uppercase tracking-[0.28em] text-muted">Measure</div><h3 className="text-xl font-bold text-main mt-2">Performance Review</h3><p className="text-sm text-muted mt-2">Keep results visible after launch.</p></div>

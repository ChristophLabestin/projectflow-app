            ...
            <div className="mb-6 flex items-center gap-3">
                <div className={`px-4 py-1.5 backdrop-blur-md rounded-full border border-white/10 flex items-center gap-2 ${viewMode === 'approved' ? 'bg-emerald-500/20 text-emerald-300' :
                    ...
                    <span className="text-[10px] font-black uppercase tracking-[0.2em]">
                        {viewMode === 'approved' ? 'Mission Active' :
                            viewMode === 'reviewer-pending' ? 'Ready for Launch' : 'Draft Mode'}
                    </span>
                </div>
            </div>
...
const StrategyTimeline: React.FC<{ phases: any[] }> = ({ phases }) => (
    <div className="bg-white/50 dark:bg-slate-900/50 backdrop-blur-xl rounded-[2rem] p-8 border border-white/50 dark:border-slate-800 shadow-xl col-span-1 lg:col-span-8">
        <h3 className="font-black text-slate-900 dark:text-white uppercase text-[11px] tracking-[.25em] mb-8">Mission Flight Path</h3>
        ...
        <div className="bg-slate-900 text-white rounded-[2rem] p-8 shadow-2xl col-span-1 lg:col-span-4 relative overflow-hidden flex flex-col justify-between min-h-[400px]">
            ...
            <div className="relative z-10">
                <div className="flex justify-between items-start mb-8">
                    <div>
                        <h3 className="font-black uppercase text-[11px] tracking-[.25em] text-indigo-300">Predictive AI</h3>
                        <p className="text-[10px] font-bold text-slate-500 mt-1">Pre-flight probability assessment</p>
                    </div>
    ...
    return (
        <div className="h-full overflow-y-auto bg-slate-50 dark:bg-slate-950 pb-32">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                    ...
                    <div className="col-span-1 lg:col-span-12 bg-white/50 dark:bg-slate-900/50 backdrop-blur-xl rounded-[2rem] p-8 border border-white/50 dark:border-slate-800 shadow-sm flex flex-wrap gap-4">
                        <div className="flex-1 min-w-[300px]">
                            <h4 className="font-black text-slate-400 uppercase text-[10px] tracking-widest mb-2">The Hook</h4>
                            <p className="text-xl font-bold text-slate-900 dark:text-white leading-tight">"{concept.hook || 'No hook defined'}"</p>
                        </div>
                        <div className="w-px bg-slate-200 dark:bg-slate-800 hidden md:block" />
                        <div className="flex-1 min-w-[300px]">
                            <h4 className="font-black text-slate-400 uppercase text-[10px] tracking-widest mb-2">Visual Direction</h4>
                            <p className="text-base font-medium text-slate-600 dark:text-slate-300 leading-relaxed">{concept.visualDirection || 'Not specified'}</p>
                        </div>
                        <div className="w-px bg-slate-200 dark:bg-slate-800 hidden md:block" />
                        <div className="flex-1 min-w-[200px]">
                            <h4 className="font-black text-slate-400 uppercase text-[10px] tracking-widest mb-2">Target Platforms</h4>
                            <div className="flex gap-2">
            ...
            <Modal
                ...
                title={rejectType === 'changes' ? "Request Mission Changes" : "Abort Mission"}
                size="md"
            >
                <div className="space-y-6">
                    <p className="text-sm font-medium text-slate-600 dark:text-slate-400 leading-relaxed">
                        {rejectType === 'changes'
                            ? "Provide specific directives for the creative team to align with mission objectives."
                            : "Confirm mission abort. This action is irreversible and will mark the campaign as rejected."}
                    </p>
                    <textarea
                    ...
                    <div className="flex justify-end gap-3">
                        ...
                        >
                            {rejectType === 'changes' ? "Transmit Feedback" : "Confirm Abort"}
                        </Button>
                    </div>
                </div>
            </Modal>

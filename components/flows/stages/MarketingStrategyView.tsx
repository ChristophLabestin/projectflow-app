        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full">
            ...
            <div className="col-span-1 flex flex-col h-full bg-white dark:bg-slate-900/50 rounded-2xl border border-[var(--color-surface-border)] shadow-sm p-6 relative overflow-hidden">
                ...
                <div className="flex flex-col h-full relative z-10">
                    <div className="mb-6">
                        <h2 className="text-xl font-extrabold text-[var(--color-text-main)] tracking-tight">{t('flowStages.marketingStrategy.title')}</h2>
                        <p className="text-xs text-[var(--color-text-muted)] mt-1">{t('flowStages.marketingStrategy.subtitle')}</p>
                        <div className="h-1 w-10 bg-pink-500 rounded-full mt-3" />
                    </div>

                    <div className="space-y-5">
                        <div>
                            <label className="text-xs font-bold text-[var(--color-text-muted)] uppercase tracking-wider mb-2 block">{t('flowStages.marketingStrategy.goal.label')}</label>
                        ...
                        <div>
                            <label className="text-xs font-bold text-[var(--color-text-muted)] uppercase tracking-wider mb-1.5 block">{t('flowStages.marketingStrategy.target.label')}</label>
                            <input
                                type="text"
                        ...
                        <div className="flex-1">
                            <label className="text-xs font-bold text-[var(--color-text-muted)] uppercase tracking-wider mb-1.5 block">{t('flowStages.marketingStrategy.message.label')}</label>
                            <textarea
            ...
            <div className="col-span-1 flex flex-col h-full bg-white dark:bg-slate-900/50 rounded-2xl border border-[var(--color-surface-border)] shadow-sm p-6 overflow-hidden">
                <div className="mb-4">
                    <h3 className="font-bold text-[var(--color-text-main)]">{t('flowStages.marketingStrategy.channels.title')}</h3>
                    <p className="text-xs text-[var(--color-text-muted)]">{t('flowStages.marketingStrategy.channels.subtitle')}</p>
                </div>

                <div className="space-y-4">
                    <div>
                        <label className="text-xs font-bold text-[var(--color-text-muted)] uppercase tracking-wider mb-2 block">{t('flowStages.marketingStrategy.channels.label')}</label>
                        <div className="flex flex-col gap-2">
                            {MARKETING_CHANNELS.map((channel) => (
                                ...
                                >
                                    <span className={`text-sm font-medium ${strategy.channels.includes(channel.id) ? 'text-pink-700 dark:text-pink-300' : 'text-[var(--color-text-main)]'}`}>
                                        {channel.label}
                                    </span>
                    ...
                    <div>
                        <label className="text-xs font-bold text-[var(--color-text-muted)] uppercase tracking-wider mb-1.5 block">{t('flowStages.marketingStrategy.budget.label')}</label>
            ...
            <div className="col-span-1 lg:col-span-1 flex flex-col h-full bg-[var(--color-surface-paper)] rounded-2xl border border-[var(--color-surface-border)] shadow-sm p-6 overflow-hidden">
                <div className="mb-4">
                    <h3 className="font-bold text-[var(--color-text-main)]">{t('flowStages.marketingStrategy.metrics.title')}</h3>
                    <p className="text-xs text-[var(--color-text-muted)]">{t('flowStages.marketingStrategy.metrics.subtitle')}</p>
                </div>

    ...
    return (
        <div className="relative" ref={dropdownRef}>
            <button
                ...
                className={`
                    group flex items-center gap-2 h-10 pl-1.5 pr-3 rounded-full transition-all border
                    ${isOpen
            ...
            >
                {/* Project Icon */}
                <div className={`
                    size-7 rounded-full flex items-center justify-center overflow-hidden shrink-0 border border-transparent group-hover:border-[var(--color-surface-border)] transition-all
            ...
            {isOpen && (
                <div className="absolute top-[calc(100%+8px)] right-0 w-80 z-50 animate-in fade-in zoom-in-95 duration-200 origin-top-right">
                    <div className="glass-card bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border border-white/20 dark:border-white/10 rounded-2xl shadow-2xl ring-1 ring-black/5 overflow-hidden">
                        ...
                        <div className="p-2 grid grid-cols-3 gap-1 bg-slate-50/50 dark:bg-black/20">
                            ...
                            {(modules.includes('ideas') || modules.length === 0) && (
                                <button
                                    onClick={() => handleAction(() => navigate(`/project/${pinnedProject.id}/ideas?create=true`))}
                                    className="flex flex-col items-center justify-center gap-1.5 p-2 rounded-xl hover:bg-white dark:hover:bg-white/10 hover:shadow-sm transition-all group/btn"
                                >
                            ...
                            {(modules.includes('issues') || modules.length === 0) && (
                                <button
                                    onClick={() => handleAction(() => navigate(`/project/${pinnedProject.id}/issues?create=true`))}
                                    className="flex flex-col items-center justify-center gap-1.5 p-2 rounded-xl hover:bg-white dark:hover:bg-white/10 hover:shadow-sm transition-all group/btn"
                                >

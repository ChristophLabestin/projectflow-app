...
struct ProjectIssueListEmbed: View {
    let tenantId: String
    let issues: [Issue]
    let permissions: PermissionContext
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: PFSpacing.sm) {
            Text("Issues").font(.headline).padding(.horizontal, PFSpacing.md)
            ForEach(issues, id: \.id) { issue in
                NavigationLink {
                    ProjectIssueDetailView(issue: issue, tenantId: tenantId, permissions: permissions)
                } label: {
                    Text(issue.title).font(.subheadline).padding(.horizontal, PFSpacing.md)
                }
            }
        }
    }
...
struct ProjectFlowListEmbed: View {
    let tenantId: String
    let flows: [Flow]
    let permissions: PermissionContext
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: PFSpacing.sm) {
            Text("Flows").font(.headline).padding(.horizontal, PFSpacing.md)
            ForEach(flows, id: \.id) { flow in
                NavigationLink {
                    FlowDetailView(flow: flow, tenantId: tenantId, permissions: permissions)
                } label: {
                    Text(flow.title).font(.subheadline).padding(.horizontal, PFSpacing.md)
                }
            }
        }
    }
